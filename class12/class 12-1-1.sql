...
-- prodduct 테이블에 is_deleted 컬럼 추가 (소프트 삭제용)
...
-- update 연습 (반드시 실수 하지 말자)
-- 김영희 등급을 gold 로 바꾸고 싶음
...
update product
set is_deleted = true -- 1 써도 됨
where id = 1;

-- 삭제 되지 않은 상품만 조회
select * from product where is_deleted = 0; -- false 써도 됨
